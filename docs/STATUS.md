...
| Real reference-corpus analysis + preset calibration (`engine/reference_vocab.py` -- MIDI/Guitar-Pro/audio input, real Krumhansl-Schmuckler key correlation, accumulating corpus cache, real preset-writing) | WIRED+TESTED | engine/ 654 passed, 1 skipped; real 31-song corpus (18 MIDI/GP tabs + 13 user-owned audio tracks via demucs+basic_pitch) -> `labyrinth.json` preset (139bpm, minor); `"boo"` alias updated; verified live -- see TASKS.md's §17.6 tracker entry, 2026-09-10 |
