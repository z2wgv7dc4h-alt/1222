...
| boo-lab GP matcher | WIRED+TESTED | `_key` strips space-numbered track + band prefixes; `_gp_candidates` scores exact/substring + lead-number + GP7 bonus; one GP → one FLAC; full Discovery/AHP/Eternal Reign GP7 packs win; `map.csv` 83 rows / 66 `match=yes`, `sync` 14/52 |
