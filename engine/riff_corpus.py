"""Real, local-only training-data pipeline for the Born-of-Osiris riff
model (see `riff_model.py`). Converts real, timed note transcriptions
(`audio_vocab.transcribe_full_note_sequence`) into a compact token
vocabulary -- real interval-class + duration-bucket pairs, plus real
bar-boundary markers -- and writes one JSON file per real song to a
LOCAL-ONLY, gitignored cache (`engine/data/ml_corpus/`, never committed:
see `.gitignore`'s own comment on why).

This is a genuine, deliberate departure from this project's established
"derived statistics only, never full ordered sequences" discipline
(`reference_vocab.py`/`midi_vocab.py`'s own committed JSON caches hold only
aggregate stats/transition COUNTS). Training a real sequence model needs
the actual real per-song token sequence, not just a bigram table -- so this
module's own output is treated with the same care as the raw audio/MIDI
corpora themselves: real, useful locally, never distributed.

Token vocabulary (small on purpose -- this project's real corpus is ~70
songs, not millions):
  - PAD = 0, BOS = 1, BAR = 2 (bar-boundary marker, real 4-beat downbeats),
    ENERGY_LOW = 3, ENERGY_MID = 4, ENERGY_HIGH = 5 -- real section-
    intensity conditioning (see `energy_bucket_from_relative_energy`),
    inserted only at real bucket TRANSITIONS (same sparse convention as
    `BAR`), so the model learns "you are now writing a high-energy
    passage" without a token on every single note.
  - Every other id encodes one real (interval_class, duration_bucket) pair,
    `_NUM_SPECIAL + interval_class * len(DURATION_BUCKETS) +
    duration_bucket_index`. `interval_class` is the SAME real 0-11
    semitone-interval-from-previous-note convention `theory.
    pick_pitch_interval_markov` already uses (transposition-invariant,
    matches the existing Markov vocabulary this project has used all
    session). `duration_bucket_index` snaps the real beat-gap to the
    nearest of `DURATION_BUCKETS` (the same real, evidence-based family
    already used for `_KICK_IOI_BUCKETS`, extended upward for guitar's own
    longer real note values).

Energy conditioning (v2, added after real listening feedback that the
v1 model had zero awareness of which section -- verse/breakdown/chorus --
it was writing for, making every section sound undifferentiated): a
real, directly measurable signal (`audio_vocab.energy_curve`, RMS
loudness relative to a song's OWN average) rather than an attempted
semantic role label. `audio_vocab.structural_segments` only gives
unsupervised boundary TIMESTAMPS, not semantic identity -- claiming it
knows "this is a breakdown" would overclaim precision this project
doesn't have. Energy sidesteps that, and the engine already has a real,
hand-authored role->energy bridge (`theory.ARC`) to connect these
training-time labels to generation-time conditioning (see
`riff_model.generate_riff_motif` and `song._try_riff_model_motif`).
"""
